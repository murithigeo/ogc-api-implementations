...
const params_1 = require("../components/params");
function makeExegesisPlugin(data, allowed_F_values, queryParamsToIgnore, listOfCollections) {
    return {
        postSecurity: async (pluginContext) => {
            ...
            if (_oasListedParams.path.collectionId === "") {
                pluginContext.res.status(400).setBody(pluginContext.makeValidationError("collectionId cannot be an empty string", {
                    in: "path",
                    name: "collectionId",
            ...
            if (_oasListedParams.query.bbox &&
                ...
                pluginContext.res.status(400).setBody(pluginContext.makeValidationError("bbox can only have 4 or 6 elements", {
                    in: "query",
                    docPath: pluginContext.api.pathItemPtr,
                    name: "bbox",
                }));
            ...
            let _url = new url_1.URL(pluginContext.api.serverObject.url + pluginContext.req.url);
            ...
            const unexpectedParams = _allParams.filter((param) => !allowedParams.includes(param));
            //Handle
            //If any undocumented params are in the array, return status(400)
            if (unexpectedParams.length > 0) {
                pluginContext.res.status(400).setBody(pluginContext.makeValidationError("Invalid query parameter", {
                    in: "query",
                    docPath: pluginContext.api.pathItemPtr,
            ...
            if (_oasListedParams.query.f &&
                !allowed_F_values.includes(_oasListedParams.query.f)) {
                pluginContext.res.status(400).setBody(pluginContext.makeValidationError("requested content-negotiation value not supported", {
                    in: "query",
                    docPath: pluginContext.api.pathItemPtr,
                    name: "f",
                }));
            ...
            if (_oasListedParams.query.crs &&
                !(await (0, params_1.validate_crs_string)(_oasListedParams.query.crs))) {
                pluginContext.res.status(400).setBody(pluginContext.makeValidationError("crs invalid or unsupported", {
                    in: "query",
                    name: "crs",
                    docPath: pluginContext.api.pathItemPtr,
                }));
            ...
            if (_oasListedParams.query["bbox-crs"]) {
                if (!(await (0, params_1.validate_crs_string)(_oasListedParams.query["bbox-crs"]))) {
                    pluginContext.res.status(400).setBody(pluginContext.makeValidationError("bbox-crs invalid or unsupported", {
                        in: "query",
                        name: "bbox-crs",
                        docPath: pluginContext.api.pathItemPtr,
                    }));
...
function validateRequestsPlugin(allowed_F_values, queryParamsToIgnore, listOfCollections) {
    return {
        info: {
            name: "exegesis-plugin-validaterequests-featuresapi",
        },
        makeExegesisPlugin: (data) => makeExegesisPlugin(data, allowed_F_values, queryParamsToIgnore, listOfCollections),
    };
}
exports.default = validateRequestsPlugin;
