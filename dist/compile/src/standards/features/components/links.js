...
exports.genLinksAll = exports.genLinksToColl_ItemsWhenAtRoot = void 0;
const url_1 = require("url");
const params_1 = __importDefault(require("./params"));
async function filter_f_types(context, allowed_f_values) {
 ...
 * @param mode
 */
async function genLinksAll(context, allowed_f_values, mode) {
    //Link to current endpoint. It has query parameters
    const { urlToThisEP } = await (0, params_1.default)(context);
    /**
    ...
    const { optionsForSelf, optionsForAlt } = await filter_f_types(context, allowed_f_values);
    /**
     * @const links
     */
    const links = [];
    ...
    for (const option of optionsForSelf) {
        ...
        let _tempLink = new url_1.URL(urlToThisEP);
    ...
    switch (mode) {
        case "Root":
            for (const option of allowed_f_values) {
                ...
                //conformance
                var _tempLink = new url_1.URL(urlToThisEP.href);
        ...
        case "Collection":
            for (let option of allowed_f_values) {
                ...
                var _tempLink = new url_1.URL(urlToThisEP);
                _tempLink.search = "";
        ...
        case "Items":
            ...
            for (let option of optionsForSelf) {
                ...
                const { nextPageOffset, prevPageOffset } = await (0, params_1.default)(context);
            ...
            for (let option of allowed_f_values) {
                var _tempLink = new url_1.URL(urlToThisEP);
                _tempLink = new url_1.URL(new url_1.URL("..", _tempLink).toString().slice(0, -1));
                _tempLink.searchParams.set("f", option.f);
                option = await changeLinkTypeForNestLinks(option, "json", "application/json");
                links.push({
                    rel: "collection",
                    type: option.type,
                    href: _tempLink.toString(),
...
exports.genLinksAll = genLinksAll;
async function addApiKeyToSearchParams(context, link) {
    if (context.security.ApiKeyAuth) {
        if (context.user.apiKey) {
            //@ts-ignore
            var _tempLink = new url_1.URL(link.href);
...
async function genLinksToColl_ItemsWhenAtRoot(context, collectionId, allowed_f_values) {
    const { urlToThisEP } = await (0, params_1.default)(context);
    const links = [];
    for (let option of allowed_f_values) {
        ...
        var _tempLink = new url_1.URL(urlToThisEP);
        _tempLink.search = "";
        _tempLink = new url_1.URL(new url_1.URL(collectionId, _tempLink + "/").toString());
        _tempLink.searchParams.set("f", option.f);
        option = await changeLinkTypeForNestLinks(option, "json", "application/json");
        links.push({
            rel: "collection",
        ...
        var _tempLink = new url_1.URL(urlToThisEP);
        _tempLink.search = "";
        _tempLink = new url_1.URL(new url_1.URL(collectionId + "/items", _tempLink + "/").toString());
        _tempLink.searchParams.set("f", option.f);
        option = await changeLinkTypeForNestLinks(option, "json", "application/geo+json");
        links.push({
            type: option.type,
...
exports.genLinksToColl_ItemsWhenAtRoot = genLinksToColl_ItemsWhenAtRoot;
async function changeLinkTypeForNestLinks(linkOption, fValue, intendedType) {
